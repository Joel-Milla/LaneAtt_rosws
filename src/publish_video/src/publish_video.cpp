//* Rclcpp needed code
#include <rclcpp/executors.hpp>
#include <rclcpp/node.hpp>
#include <rclcpp/publisher.hpp>
#include <rclcpp/utilities.hpp>

//* Messages libraries
#include "cv_bridge/cv_bridge.h"
#include <sensor_msgs/image_encodings.hpp> // Add this line!
#include <sensor_msgs/msg/detail/image__struct.hpp>

//* Opencv libraries
#include "geometry_msgs/msg/accel.hpp"
#include <opencv2/core.hpp>
#include <opencv2/highgui.hpp>
#include <opencv2/imgproc.hpp>
#include <opencv2/videoio.hpp>

class PublishImage : public rclcpp::Node {
private:
  static constexpr const char *NODE_NAME = "video_publisher";
  static constexpr const char *PUB_IMG_TOPIC = "video_publisher";
  static constexpr const int STANDARD_QOS = 10;
  static constexpr const int FPS = 15;
  static constexpr const char *VIDEO_PATH =
      "/home/joel/Documents/research/RealTime-LaneATT/realsense/videos/"
      "testeo_final_5.avi";

  using Image = sensor_msgs::msg::Image;

  rclcpp::Publisher<Image>::SharedPtr img_pub_;
  rclcpp::TimerBase::SharedPtr timer_;
  cv::VideoCapture cap_;
  bool video_opened_ = false;

  void timer_callback() {
    if (!video_opened_) {
      cap_.open(VIDEO_PATH);
      if (!cap_.isOpened()) {
        RCLCPP_ERROR(this->get_logger(), "Unable to open video file");
        rclcpp::shutdown();
        return;
      }
      video_opened_ = true;
    }

    cv::Mat frame;
    cap_.read(frame);

    if (frame.empty()) {
      RCLCPP_INFO(this->get_logger(), "Video ended, restarting...");
      cap_.set(cv::CAP_PROP_POS_FRAMES, 0); // Restart video
      return;
    }

    cv_bridge::CvImage cv_image;
    cv_image.header.stamp = this->get_clock()->now();
    cv_image.header.frame_id = "camera_frame";
    cv_image.encoding = sensor_msgs::image_encodings::BGR8;
    cv_image.image = frame;

    img_pub_->publish(*cv_image.toImageMsg());
  }

public:
  explicit PublishImage() : Node(NODE_NAME) {
    img_pub_ = this->create_publisher<Image>(PUB_IMG_TOPIC, STANDARD_QOS);

    auto period = std::chrono::milliseconds(1000 / FPS);
    timer_ = this->create_wall_timer(
        period, std::bind(&PublishImage::timer_callback, this));
  }

  ~PublishImage() {
    if (cap_.isOpened()) {
      cap_.release();
    }
  }
};

int main(int argc, char **argv) {
  rclcpp::init(argc, argv);
  auto node = std::make_shared<PublishImage>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}